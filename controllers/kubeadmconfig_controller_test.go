...
func TestKubeadmConfigReconciler_ReturnEarlyIfClusterInfraNotReady(t *testing.T) {
	...
	if actualResult.Requeue != expectedResult.Requeue {
		t.Fatalf("TestKubeadmConfigReconciler_ReturnEarlyIfClusterInfraNotReady failed, reconcile result doesn't match, Want %v, Got %v", expectedResult, actualResult)
	}
	if actualResult.RequeueAfter != expectedResult.RequeueAfter {
		t.Fatalf("TestKubeadmConfigReconciler_ReturnEarlyIfClusterInfraNotReady failed, reconcile result doesn't match, Want %v, Got %v", expectedResult, actualResult)
	}

	if actualError != nil {
		t.Fatalf("TestKubeadmConfigReconciler_ReturnEarlyIfClusterInfraNotReady failed, error doesn't match expected value, Want nil, Got %v", actualError)
	}
}
